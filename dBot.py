...
async def on_message(message):  # Viestin tullessa kanavalle
    ...
    if "sää" in message.content.lower():  # Säähaku
        ...
        api_address = 'http://api.openweathermap.org/data/2.5/weather?appid=0c42f7f6b53b244c78a418f4f181282a&q=' + msg[
            1]
    ...
    if "!twitch" in message.content.lower():
        headers = {
            'Client-ID': 'q25bv6w1pjmxp783i9ms6n0cw8f1ko',
        }
        ...
        for streamer in json_data['data']:
            twitch_message += "**" + str(streamer['user_name']) + "**" + " - " + game_name_dict[
                streamer['game_id']] + " - " + str(streamer['viewer_count']) + "\n"
